...
async fn main() -> Result<()> {
    ...
    let email = EmailAddress::from_str(&args[1]).context("Invalid email address")?;
    ...
    storage_adapter.upsert_subscriber(&subscriber).await?;

    println!("Successfully added subscriber:");
